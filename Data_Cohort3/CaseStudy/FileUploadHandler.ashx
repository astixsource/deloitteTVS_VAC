...
public class FileUploadHandler : IHttpHandler
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        try
        {
            ...
            else if (context.Request.QueryString["flgfilefolderid"].ToString() == "8") //For multiple files functionality
            {
                ...
                int RSPExerciseID = int.Parse(context.Request.Form["RSPExerciseID"].ToString());
                int LoginId = int.Parse(context.Request.Form["LoginId"].ToString());
                int FileTypeID = int.Parse(context.Request.Form["FileTypeID"].ToString());
                string Savepath = context.Server.MapPath("~/" + Serverpath);
                ...
                try
                {
                    for (int i = 0; i < context.Request.Files.Count; i++)
                    {
                        ...
                        fileWithoutExt = Path.GetFileNameWithoutExtension(postedFile.FileName).Replace(" ","");
                    ...
                    // ObjclassUsedForExerciseSave.spRspSaveResponseFileName(FileTypeID,RSPExerciseID, LoginId, fileWithoutExt, 1, context.Request.Files);
                   // ObjclassUsedForExerciseSave.spRspSaveResponseFileName(FileTypeID,RSPExerciseID, LoginId, fileWithoutExt, 1);
             ...
             else if (context.Request.QueryString["flgfilefolderid"].ToString() == "9") //For multiple files functionality
            {
                //for uploading new File
                string Serverpath = context.Request.Form["FolderName"].ToString();
                int ParticipantID = int.Parse(context.Request.Form["ParticipantID"].ToString());
                int CycleID = int.Parse(context.Request.Form["CycleId"].ToString());
                ...
                try
                {
                    for (int i = 0; i < context.Request.Files.Count; i++)
                    {
                        ...
                        decimal filesize = Math.Round(((decimal)postedFile.ContentLength / (decimal)1024), 2);
                        fileWithoutExt = fileWithoutExt + "_" + ParticipantID + fileExt;
                        string fileDirectory = Savepath + "\\" + fileWithoutExt;
                        postedFile.SaveAs(fileDirectory);
                    }
                    ...
                    classUsedForExerciseSave ObjclassUsedForExerciseSave = new classUsedForExerciseSave();
                    // ObjclassUsedForExerciseSave.spRspSaveResponseFileName(FileTypeID,RSPExerciseID, LoginId, fileWithoutExt, 1, context.Request.Files);
                    //ObjclassUsedForExerciseSave.fnUpdateFileNameForIDPTemplate(ParticipantID, CycleID, fileWithoutExt);
            ...
            else
            {
               ...
               // ObjclassUsedForExerciseSave.spRspSaveResponseFileName(1,RSPExerciseID, LoginId, file, Status);

...
public partial class Admin_MasterForms_frmDownloadCareerReflection : System.Web.UI.Page
{
    ...
    public static string fnGetEntries(string CycleId)
    {
        try
        {
            ...
            SqlCommand Scmd = new SqlCommand();
            Scmd.Connection = Scon;
            Scmd.CommandText = "spGetListOfUsersSubmittedCareerReflectionForm";
            Scmd.CommandType = CommandType.StoredProcedure;
    ...
    private static string CreateSchedulingTbl(DataTable dt, string[] SkipColumn, string tblname, int RowMerge_Index)
    {
        ...
        for (int i = 0; i < dt.Rows.Count; i++)
        {
                ...
                if (!SkipColumn.Contains(dt.Columns[j].ColumnName.ToString().Trim()))
                {
                    ...
                    else
                    {
                        ...
                        else if (dt.Columns[j].ColumnName.ToString() == "Status")
                        {
                            ...
                            if (Status == "1")
                            {
                                sb.Append("<td class='cls-" + j + "'>Save</td>");
                            }
                            else
                            {
                                sb.Append("<td class='cls-" + j + "'>Submit</td>");
                            }
                        }
                        else
                        {
            ...
            sb.Append("<td class='cls-6'><a href='../../frmDownloadExcel.aspx?flg=2&EmpNodeID=" + dt.Rows[i]["EmpNodeID"].ToString() + "' target='_blank'  class='fa fa-download' style='padding:2px 4px;font-size:9pt'></a></td>");

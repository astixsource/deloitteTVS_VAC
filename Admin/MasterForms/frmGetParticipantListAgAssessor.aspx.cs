...
public partial class Admin_Evidence_frmGetParticipantListAgAssessor : System.Web.UI.Page
{
    ...
    public static string fnGetParticipantDetails(int CycleID, int loginId, int flgCallType)
    {
        ...
        else
        {
            ...
            //DataRow[] drow = ds.Tables[0].Select("[Status] like '%Discussion To Start%' OR [Status]='Preparation In Progress' OR [Status]='Discussion Over'");
            //if (drow.Length > 0)
            //{
            strResponse = JsonConvert.SerializeObject(ds, Formatting.Indented, new JsonSerializerSettings { ReferenceLoopHandling = ReferenceLoopHandling.Ignore });
            //}
        }
        return strResponse;
    }
...
SkipColumn[32] = "DCType";
        ...
        for (int k = 0; k < Collength.Length; k++)
        {
                ...
                if (!SkipColumn.Contains(dt.Columns[j].ColumnName.ToString().Split('|')[0].Trim()))
                {
                    ...
                    if (ColSpliter[k] != "")
                    {
                        if (string.Join("", ColSpliter) == ColSpliter[k])
                        {
                            ...
                            {
                                sb.Append("<th rowspan='" + ColSpliter.Length + "' style='width:30%' >" + ColSpliter[k] + "</th>");
                            }
                            else
                            {
            ...
            sb.Append("<th style='width:20%'>Action</th>");
            sb.Append("</tr>");
        }
        sb.Append("</thead>");
                ...
                for (int j = 0; j < ds.Tables[0].Columns.Count; j++)
                {
                    if (!SkipColumn.Contains(ds.Tables[0].Columns[j].ColumnName.ToString()))
                    {
                        ...
                        else
                        {
                            ...
                            else if (ds.Tables[0].Columns[j].ColumnName == "Participant Name" && Row["DCType"].ToString()!="")
                            {
sb.Append("<td style='vertical-align:middle'>" + Row[ds.Tables[0].Columns[j].ColumnName].ToString() + "</td>");
                                //sb.Append("<td iden='mast' style='vertical-align:middle'><a href='###' onclick=\"fnSHowCR('"+ Row["ParticipantId"].ToString() + "','"+ Row["DCType"].ToString() + "')\" title='Click to view participant CR form'>" + Row[ds.Tables[0].Columns[j].ColumnName].ToString() + "</a></td>");
                            }
                ...
                if (RuleId==3 && flg == "0")
                {
                    sresumeLink ="<a href='###' onclick='fnStartMeeting(this)'  flg='3' rspexerciseid='" + Row["rspexerciseid"].ToString() + "' gotousername='" + Convert.ToString(Row["gotousername"]) + "' gotopassword='" + Convert.ToString(Row["gotopassword"]) + "' meetingid='" + Convert.ToString(Row["meetingid"]) + "'  flgrule='" + Convert.ToString(Row["flgrule"]) + "'  class='btn btn-primary clsaction' style='padding:0px 4px;font-size:8pt;margin-left:5px;' AssessorMeetingLink='" + Convert.ToString(Row["AssessorMeetingLink"]) + "'  >Resume Meeting</a>";
                }
                else
                {
                    sresumeLink = (RuleId == 4 || RuleId ==5 || RuleId==6 || flg == "1") ?"": "<a href='###' onclick='fnStartMeeting(this)' flg='3' rspexerciseid='" + Row["rspexerciseid"].ToString() + "' gotousername='" + Convert.ToString(Row["gotousername"]) + "' gotopassword='" + Convert.ToString(Row["gotopassword"]) + "' meetingid='" + Convert.ToString(Row["meetingid"]) + "'  flgrule='" + Convert.ToString(Row["flgrule"]) + "' AssessorMeetingLink='" + Convert.ToString(Row["AssessorMeetingLink"]) + "'   class='btn btn-primary clsaction' style='padding:0px 4px;font-size:8pt;margin-left:5px;display:none' >Resume Meeting</a>";
                }
                if (Row["flgShowLink"].ToString() == "1")
                {
                    if (Row_Span1 > 1)
                    {
                        if (Row == drows1)
                        {
                            sb.Append("<td style='vertical-align:middle' " + stitle + " rowspan='"+ Row_Span1 + "' class='cellaction'><a href='###' onclick='fnStartMeeting(this)' flg='" + flg + "' rspexerciseid='" + Row["rspexerciseid"].ToString() + "' AssessorMeetingLink='" + Convert.ToString(Row["AssessorMeetingLink"]) + "' meetingid='" + Convert.ToString(Row["meetingid"]) + "' gotousername='" + Convert.ToString(Row["gotousername"]) + "' gotopassword='" + Convert.ToString(Row["gotopassword"]) + "'  flgrule='" + Convert.ToString(Row["flgrule"]) + "' class='btn btn-primary clsaction' style='padding:0px 4px 0px 0px;font-size:8pt' >" + Convert.ToString(Row["MeetingButtonName"]) + "</a>"+ sresumeLink + "</td>");
                        }
                    }
                    else
                    {
                        sb.Append("<td style='vertical-align:middle' " + stitle + "  class='cellaction'><a href='###' onclick='fnStartMeeting(this)' flg='" + flg + "' rspexerciseid='" + Row["rspexerciseid"].ToString() + "' AssessorMeetingLink='" + Convert.ToString(Row["AssessorMeetingLink"]) + "' meetingid='" + Convert.ToString(Row["meetingid"]) + "'  gotousername='" + Convert.ToString(Row["gotousername"]) + "' gotopassword='" + Convert.ToString(Row["gotopassword"]) + "'  flgrule='" + Convert.ToString(Row["flgrule"]) + "' class='btn btn-primary clsaction' style='padding:0px 4px 0px 0px;font-size:8pt' >" + Convert.ToString(Row["MeetingButtonName"]) + "</a>" + sresumeLink + "</td>");
                    }
                        
                }
                else
                {
                    string strbtns = (RuleId == 5 || RuleId == 6) ? Convert.ToString(Row["MeetingButtonName"]) : "<a href ='###' onclick ='fnStartMeeting(this)' flg='" + flg + "' rspexerciseid ='" + Row["rspexerciseid"].ToString() + "' AssessorMeetingLink='"+ Convert.ToString(Row["AssessorMeetingLink"]) + "' meetingid ='" + Convert.ToString(Row["meetingid"]) + "' gotousername ='" + Convert.ToString(Row["gotousername"]) + "' gotopassword ='" + Convert.ToString(Row["gotopassword"]) + "'  flgrule ='" + Convert.ToString(Row["flgrule"]) + "'  class='btn disabled clsaction'  style='padding:0px 4px 0px 0px;font-size:8pt'>" + Convert.ToString(Row["MeetingButtonName"]) + "</a>" + sresumeLink;
                    if (Row_Span1 > 1)
                    {
                        if (Row == drows1)
                        {
                           
                            sb.Append("<td style='vertical-align:middle' " + stitle + " rowspan='"+ Row_Span1 + "'  class='cellaction'>" + strbtns + "</td>");
                        }
                    }
                    else
                    {
                        sb.Append("<td style='vertical-align:middle' " + stitle + "  class='cellaction'>" + strbtns + "</td>");
                    }
    ...
    [System.Web.Services.WebMethod()]
    public static string fnUpdateActualStartEndTime(string RSPExerciseid, string Exerciseid, int UserTypeID, string flgAction,string GotoUserName,string GotoPassword,string Meetingid,int flgType,int RspId,int LoginId,int CycleId,string MeetingScheduledStartTime)
    {
        try
        {
            if (flgType != 3)
    ...
    [System.Web.Services.WebMethod()]
    public static string fnCreateGDExercises(string RSPExerciseid, string Exerciseid, int UserTypeID, string flgAction, int RspId, int LoginId, int CycleId, string MeetingScheduledStartTime)
    {
        try
        {
                ...
                Scmd = new SqlCommand();
                Scmd.Connection = Scon;
                Scmd.CommandText = "[spCreateRSPExerciseForType2MeetingByAssessor_New]";
                Scmd.CommandType = CommandType.StoredProcedure;
                Scmd.Parameters.AddWithValue("@ExerciseId", Exerciseid);
                Scmd.Parameters.AddWithValue("@LoginId", LoginId);
                Scmd.Parameters.AddWithValue("@CycleId", CycleId);
                ...
                Scmd.CommandTimeout = 0;
                Scon.Open();
                int strrep=(int)Scmd.ExecuteScalar();
                Scon.Close();
                Scon.Dispose();
